...
class TrackLoader:
    ...
    def load_tracks_from_db(self, sql_file, is_grid=False, is_circular=False):
        ...
        if is_grid:
            activities = (
                session.query(Activity).filter(Activity.summary_polyline != "").filter(Activity.type.not_in(["Flight"])).order_by(Activity.start_date_local)
            )
        elif is_circular:
            activities = session.query(Activity).filter(Activity.type.not_in(["RoadTrip", "Flight"])).order_by(Activity.start_date_local)
        else:
            activities = session.query(Activity).filter(Activity.type.not_in(["Flight"])).order_by(Activity.start_date_local)
        tracks = []
    ...
    def _load_data_tracks(file_names, load_func=load_gpx_file, activity_title_dict={}):
        ...
        with concurrent.futures.ProcessPoolExecutor() as executor:
            future_to_file_name = {executor.submit(load_func, file_name, activity_title_dict): file_name for file_name in file_names}
